...
export const WebpToPdf = () => {
  ...
  return (
    <div className="min-h-[calc(100vh-200px)]">
      <PageSeo title="WebP to PDF Online – Free & Private" description="Convert WebP images to PDF in your browser. No uploads, 100% private." faqItems={[{ question: 'Can I convert multiple images?', answer: 'Yes. Select multiple files to create a multi-page PDF.' }, { question: 'Is quality preserved?', answer: 'Images are embedded at their original resolution.' }]} />
      <div className="page-header"><div className="container">
        <div className="flex items-center justify-center gap-3 mb-4"><div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-teal-500 to-cyan-600 text-white flex items-center justify-center shadow-lg"><FileImage className="w-6 h-6" /></div></div>
        <h1>WebP to PDF Online</h1><p>Convert WebP images to PDF instantly in your browser.</p>
      </div></div>
      <div className="container pb-12"><div className="max-w-2xl mx-auto">
        <div className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-sm">
          <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-border rounded-xl cursor-pointer hover:bg-muted transition-colors">
            ...
            <span className="text-sm text-[var(--text-muted)]">Click to select WebP files</span>
            <input type="file" accept=".webp" multiple className="hidden" onChange={handleFiles} />
          </label>
          {files.length > 0 && (
