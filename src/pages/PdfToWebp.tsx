...
export const PdfToWebp = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      const blob = await rasterizePDF(file, 'webp', dpi, p => setProgress(p));
      await downloadBlob2(blob, file.name.replace('.pdf', '_webp_images.zip'));
  ...
  return (
    <div className="min-h-[calc(100vh-200px)]">
      <PageSeo title="PDF to WebP Online – Free & Private" description="Convert every PDF page to WebP images. Download as ZIP. 100% browser-based." faqItems={[{question:'How are images delivered?',answer:'All pages are zipped for easy download.'},{question:'What DPI should I use?',answer:'150 DPI is good for web; 300 DPI for print-quality.'}]} />
      <div className="page-header"><div className="container">
        <div className="flex items-center justify-center gap-3 mb-4"><div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-teal-500 to-green-600 text-white flex items-center justify-center shadow-lg"><FileImage className="w-6 h-6" /></div></div>
        <h1>PDF to WebP Online</h1><p>Convert every page to a WebP image and download as a ZIP.</p>
      </div></div>
        ...
        <div className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-sm">
          ...
          {file && (
            <div className="mt-6 animate-fade-in space-y-4">
              ...
              {isProcessing && <div className="space-y-2"><div className="flex justify-between text-sm"><span>Converting…</span><span>{Math.round(progress)}%</span></div><div className="h-2 rounded-full bg-muted overflow-hidden"><div className="h-full bg-gradient-to-r from-teal-500 to-green-600 transition-all" style={{ width: `${progress}%` }} /></div></div>}
              ...
              <button onClick={handleProcess} disabled={isProcessing} className="btn btn-primary w-full py-4 text-lg disabled:opacity-50">
                {isProcessing ? <><Loader2 className="w-5 h-5 animate-spin" />Converting…</> : <><Download className="w-5 h-5" />Convert to WebP &amp; Download ZIP</>}
              </button>
            </div>
          )}
        </div>
